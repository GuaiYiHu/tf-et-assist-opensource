...
class PafNet:
    def __init__(self, inputs_x, use_bn=False, mask_paf=None, mask_hm=None, gt_hm=None, gt_paf=None, stage_num=6,
                 hm_channel_num=16, paf_channel_num=30):
    ...
    def gen_net(self):
        ...
        with tf.variable_scope('openpose_layers'):
            ...
            with tf.variable_scope('cpm_layers') as scope:
                added_layers_out.append(self.add_layers(inputs=self.inputs_x[0]))
            for i in range(self.stage_num - 2):
                with tf.variable_scope(scope, reuse=True):
                    added_layers_out.append(self.add_layers(inputs=self.inputs_x[i + 1]))
            with tf.variable_scope('stage_1'):
                paf_net = self.stage_1(inputs=added_layers_out[0], out_channel_num=self.paf_channel_num,
                                       name='stage1_paf')
                hm_net = self.stage_1(inputs=added_layers_out[0], out_channel_num=self.hm_channel_num, name='stage1_hm')
                paf_pre.append(paf_net)
                hm_pre.append(hm_net)
                net = tf.concat([hm_net, paf_net, added_layers_out[0]], 3)
            ...
            for i in range(self.stage_num - 1):
                with tf.variable_scope('stage_%d' % (i + 2)):
                    ...
                    if i < self.stage_num - 2:
                        net = tf.concat([hm_net, paf_net, added_layers_out[i + 1]], 3)
    ...
    def gen_hand_net(self):
        ...
        with tf.variable_scope('openpose_layers'):
            ...
            with tf.variable_scope('stage1'):
                ...
                hm_pre.append(hm_net)
                net = tf.concat([hm_net, added_layers_out], 3)
        ...
        return hm_pre, added_layers_out
