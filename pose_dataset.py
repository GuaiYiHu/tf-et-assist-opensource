...
from pose_augment import pose_rotation, pose_to_img, pose_crop_random, hand_pose_to_img, \
    pose_resize_shortestedge_random, pose_resize_shortestedge_fixed, pose_crop_center, pose_random_scale, pose_gen_mask
...
class CocoMetadata:
    ...
    def __init__(self, idx, img_url, img_meta, annotations, miss_mask, sigma):
        self.idx = idx
...
class HandMetadata:
    ...
    def parse_floats(four_nps, adjust=0):
        assert len(four_nps) % 4 == 0
        return [(HandMetadata.parse_float(four_nps[x * 4:x * 4 + 4]) + adjust) for x in
                range(len(four_nps) // 4)]
    ...
    def get_heatmap(self, target_size):
        heatmap = np.zeros((HandMetadata.__coco_parts, self.height, self.width), dtype=np.float32)

        for joints in self.joint_list:
            for idx, point in enumerate(joints):
                if point[0] < 0 or point[1] < 0:
                    continue
                HandMetadata.put_heatmap(heatmap, idx, point, self.sigma)
...
class HandPose(RNGDataFlow):
    ...
    def __init__(self, img_path=None, is_train=True, decode_img=True, only_idx=-1, input_size=368):
        self.is_train = is_train
        self.decode_img = decode_img
        self.only_idx = only_idx
        self.img_path = img_path
        self.input_size = input_size
        self.img_list = [i for i in os.listdir(self.img_path) if i.split('.')[-1] == 'jpg'][11:]
        logger.info('%s dataset %d' % (img_path, self.size()))
    ...
    def get_data(self):
        ...
        for idx in idxs:
            ...
            img_meta = {'file_name': file_name, 'height': self.input_size, 'width': self.input_size}
            img_url = os.path.join(self.img_path, file_name)

            anns = [{'num_keypoints': 1, 'keypoints': [self.input_size / 2, self.input_size / 2, 1]}]
            meta = HandMetadata(idx, img_url, img_meta, anns, sigma=8.0)
            try:
                meta.img = cv2.resize(cv2.imread(img_url), (self.input_size, self.input_size))
            except:
                print(img_url)
...
class PoseTrackPose(RNGDataFlow):
    ...
    def get_data(self):
        ...
        for anno_file_idx in anno_file_idxs:
            coco = COCO(self.whole_path + anno_file_idx)
...
def get_dataflow(path, is_train, img_path=None):
    ...
    if is_train:
        ...
        ds = MapDataComponent(ds, pose_gen_mask)
        ...
        ds = PrefetchData(ds, 100, multiprocessing.cpu_count() - 2, )
    else:
...
if __name__ == '__main__':
    ...
    df = get_hand_dataflow(True, '/run/user/1000/gvfs/smb-share:server=server,share=data/yzy/'
                                 'dataset/Realtime_Multi-Person_Pose_Estimation-master/'
                                 'training/dataset/COCO/images/hand/')
    ...
    with tf.Session() as sess:
        ...
        for idx, dp in enumerate(df.get_data()):
            ...
            print(time.time() - t1)
            t1 = time.time()
            # HandPose.display_image(dp[0], dp[1].astype(np.float32))
            print(dp[1].shape)
            pass
