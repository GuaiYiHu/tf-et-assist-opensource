...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str, default='checkpoints/train/fine_tone_on_pose_track/')
    ...
    parser.add_argument('--video', type=str, default='images/平举.mp4')
    ...
    net = PafNet(inputs_x=vgg_outputs, hm_channel_num=15,
                 paf_channel_num=26, use_bn=args.use_bn)
    ...
    with tf.Session(config=config) as sess:
        ...
        saver.restore(sess, args.checkpoint_path + 'model-65000')
        ...
        if args.image is None:
            ...
            if args.save_video is not None:
                ...
                video_saver = cv2.VideoWriter('result/test4_on_org.mp4', fourcc, fps, (ori_w, ori_h))
                logger.info('record vide to %s' % args.save_video)
            logger.info('fps@%f' % fps)
            size = [int(640 * (ori_h / ori_w)), 640]
            h = int(640 * (ori_h / ori_w))
            time_n = time.time()
            while True:
                _, image = cap.read()
                img = np.array(cv2.resize(image, (640, h)))
                cv2.imshow('raw', img)
